...
def phi(n):
    ps = sympy.primefactors(n)
    for p in ps:
        n = int(n*(1-1/p))
    return n

def solve(MNr="1234567"):
    x = int(MNr[2::-1])
    y = int(MNr[:3:-1])
    print('x=',x,phi(x))
    print('y=',y,phi(y))
...
def satisfysIdentity(G, Id, ACWorks=False, Idempotent=True,timelimit = float('inf'),partition=None,conservative=False,returnPoly=False):#,diagonal=False):
    #timeInd = time.time()
    ...
    return IG,q,ArcCon.findHom(IG,G,fMap,ACWorks,timelimit=timelimit)
...
Sigg3 = Identity([['p011','q100','q001'],['p010','q010']],'Sig3')
...
def getStraigtheningChain(n):
    print('does not work as intendet')
    global  alph
    vars = '0123456789'
    ids = [[alph[0] + 'aa'+ vars[:n], alph[0] + vars[0]*(n+2)]]
    ids += [[alph[0] + 'aa'+ vars[:n], alph[1] + 'bb'+ vars[:n]]]
    for i in range(n):
        ids += [[alph[i+1]+'a'+vars[i]+vars[:i]+'a'+vars[i+1:n],alph[i+2]+'bb'+vars[:n]]]
    ids += [[alph[n+1] + 'aa'+ vars[:n], alph[n+1] + vars[n-1]*(n+2)]]
    print(ids)
    return Identity(ids,'Straight'+str(n))
#noname terms
def getNN(n):
    global  alph
    i= 0
    ids = [[alph[i] + 'xyyz', alph[i] + 'xxxx']]
    for i in range(n):
        ids += [[alph[i]+'xxyx',alph[i+1]+'xyyx']]
        ids += [[alph[i]+'xxyy',alph[i+1]+'xyyy']]
    ids += [[alph[n]+'xxyz',alph[i]+'zzzz']]
    return Identity(ids,'NN'+str(n))

def getNU(n):
    Ids = ['f'+'x'*n] + ['f'+'x'*k+'y'+'x'*(n-k-1) for k in range(n)]
    return Identity([Ids],'NU'+str(n))
...
def notSatisfysIdentityMinimalSubgraph(G, Id, ACWorks=False, Idempotent=True, shouldNotContain=None):
    if shouldNotContain is None:
        shouldNotContain = set()
    IG, q = getIndicatorGraph(G, Id, returnq=True)
    fMap = None
    if Idempotent:
        fMap = dict()
        for f in Id.functions:
            a = Id.arity[f]
            fMap.update({q[tuple([f] + [u] * a)]: {u} for u in G.nodes})
    IG.remove_nodes_from([q[v] for v in shouldNotContain])
...
def printCommutativePoly(f,G,sameLevel=True):
    if sameLevel:
        levels = 2
        lvl = ArcCon.findHom(G, pptester.getPath('1' * levels))
        while lvl is None:
            levels += 1
            lvl = ArcCon.findHom(G, pptester.getPath('1' * levels))
    ...
    for r in nodes:
        rText=str(r)+' '*(2-len(str(r)))
        for c in nodes:
            if sameLevel and lvl[c]!=lvl[r]:
                rText+=', *'
            else:
                rText+=','+' '*(2-len(str(f[frozenset({('f', r, c), ('f', c, r)})])))+str(f[frozenset({('f', r, c), ('f', c, r)})])
        print(rText)
