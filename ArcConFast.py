...
def getMinimalNoHomEdgeSubstructure(G,H,ACWorks=False):
    Gsmall = getMinimalNoHomSubgraphFast(G,H,ACWorks=ACWorks)
    ess = Gsmall.edges()
    essNew = copy.deepcopy(ess)
    for i,es in enumerate(ess):
        for j,e in enumerate(es):
            essNew[i].remove(e)
            if existsHom(Structures.Structure(essNew),None, H, ACWorks, componentwise=False):
                essNew[i] +=[e]
    return Structures.Structure(essNew)
...
def getMinimalNoHomSubgraphFast(G: nx.DiGraph, H, f=None, ACWorks=False, dropRatio=0.3,timelimit=2):
    if f is None:
        f=dict()
    if isinstance(G,nx.DiGraph):
        C = nx.weakly_connected_components(G)
    ...
    for Svs in C:
        ...
        while dropRatio*len(S.nodes) > 5 and len(S.nodes) > 50:
            ...
            try:
                if not existsHom(S2, {v: g[v] for v in g if v in S2.nodes}, H, ACWorks, componentwise=True,timelimit=timelimit):
                    S = S2
                    g = {v: g[v] for v in g if v in S.nodes}
...
def existsHom(G, f, H, ACWorks=False, componentwise=True, timelimit=float('inf'),workingSet=None):
    if componentwise:
        #compute components
        if isinstance(G, Structures.Structure):
            ess = []
            for es in G.edges():
                ess += es
            GE = nx.DiGraph(ess)
            C = nx.weakly_connected_components(GE)
        else:
            C = nx.weakly_connected_components(G)
        for Svs in C:
            ff = dict()
            ...
            if workingSet is not None:
                workingSett = {v for v in S.nodes if v in workingSet}
            if not existsHom(S, ff, H, ACWorks, False, timelimit,workingSett):
                return False
        return True
    else:
        if ACWorks:
    ...
    if not isinstance(G,nx.DiGraph):
        ...
        while True:
            ...
            for i in range(len(G.Gs)):
                g, _, _ = arcCon(G.Gs[i], g, H.Gs[i],workingSet=workingSet)
            #print(g)
            if g == f:
                return f, None, None
            else:
        ...
        while len(workingSet) > 0 or len(highPworkingSet) > 0:
            #maxWorkingsetSize = max(maxWorkingsetSize,len(workingSet)+len(highPworkingSet))
            #print(len(workingSet),len(highPworkingSet))
            # print(len(dyn.keys()))
            # print(workingSet,f)
            ...
            for uIn in G.predecessors(v):
                if uIn not in f or not PossibleInVertices.issuperset(f[uIn]):
                    ...
                    if len(f[uIn]) == 0:
                        return {v: set() for v in G.nodes}, dyn, None
        #print('maxWorkingsetSize',maxWorkingsetSize)
        return f, dyn, None
...
def findHom(G, H, f=None, ACworks=False, timelimit=float("inf"), workingSet=None):
    # init f
    if workingSet is None:
        workingSet = set()
    if ACworks:
    ...
    if not f: #f = dict()
        f = initF(G, H, f)
    res = canExtend(G, f, H, ACworks, timelimit=timelimit,workingSet=workingSet)
...
def getAllPoly(G,n):
    ...
    if isinstance(G,nx.DiGraph):
        return getAllHom(pptester.pppower(G, (alph[:n], alph[:n].upper())), G)
    return getAllHom(pptester.pppower(G, [(alph[:n], alph[:n].upper())]*len(G.Gs)), G)


def canExtend(G, f, H, ACworks=False, im=None, gin=None, gout=None, timelimit=float("inf"), depth=0, debug=False,maxBreitensuche=2,workingSet=None):
    if workingSet is None:
        workingSet = set()
    maxdepth = 1
    f, gin, gout = arcCon(G, f, H, gin, gout)
    ...
    if debug:
        print('greedy search')
    try:
        g = canExtendFast(G, g, H,ACworks=True,gin=ggin,gout=ggout, timelimit=timelimit*1/3, depth=depth + 1, debug=debug, maxdepth=1000,workingSet=workingSet)
        if g is not None:
            return g
    except:
        print('timeout')
    try:
        ...
        while True:
            ...
            ggin = copy.deepcopy(gin)
            ggout = copy.deepcopy(gout)
            g = canExtendFast(G, g, H, ACworks, gin=ggin,gout=ggout,timelimit=timelimit*1/2 if timelimit < float('inf') else 3, depth=depth + 1, debug=debug, maxdepth=maxdepth,workingSet=workingSet)
            if g != 'tooDeep':
                return g
        ...
        try:
            ...
            g = canExtendGreedy(G, g, H, ACworks=True,gin=ggin,gout=ggout, timelimit=timelimit*2/3, depth=depth + 1, debug=debug,
                                maxdepth=len(G.nodes),workingSet=workingSet)
            if g is not None:
                return g
        except:
            if debug:
        ...
        ggin = copy.deepcopy(gin)
        ggout = copy.deepcopy(gout)
        return canExtendFast(G, g, H, ACworks, gin=ggin,gout=ggout,timelimit=timelimit, depth=depth + 1, debug=debug, maxdepth=len(G.nodes),workingSet=workingSet)
        ...
        for i, k in enumerate(ks):
            ...
            if len(f[k]) > 1:
                ...
                for u in [v for v in f[k] if v in im] + [v for v in f[k] if v not in im]:
                    ...
                    #now its really greedy
                    ff = f#copy.deepcopy(f)
                    ggin = gin#copy.deepcopy(gin)
                    ggout = gout#copy.deepcopy(gout)
